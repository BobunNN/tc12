...
from src.app.dependencies import SessionDep, get_current_user
...
@router.get("/v1/training-sessions")
...
def delete_training_session(
    ...
    return training_session_service.delete_training_session(session, session_id)
...
async def create_training_sessions_bulk(
    ...
    if not training_schedule_csv.filename.endswith('.csv'):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="File must be a CSV")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="Invalid file encoding. Must be UTF-8.")
